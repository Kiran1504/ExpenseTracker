...
initializePassport(
    passport, 
    email => User.find({email : User.email}),
    id => User.find({_id: User._id})
)
app.use(flash())
app.use(session({
...
let productarray;
var exana;
// console.log(tablehtml.indexOf('end-->'));
var maintable = tablehtml.slice(3067,3905);
var finaltable
// console.log(maintable);
// var logoutbtn= tablehtml.slice(1039,1105)
    ...
    User.find({byuser:byuser}).then((datas)=>{
        fs.writeFile('data.json',JSON.stringify(datas), function (err) {
            if (err){console.log(err);}
                ...
                // finaltable = tablehtml.replace(logoutbtn,'<a href="/logout">Logout</a>')
                var products = JSON.parse(fs.readFileSync('./data.json','utf-8'))
                productarray = products.map((prod)=>{
                let content = maintable.replace('{{%DATE%}}',prod.createdAt);
                content = content.replace('{{%CATEGORY%}}',prod.category)
...
app.post('/login',passport.authenticate("local",{ /// new trail code as a middleware
    successRedirect:"/",
...
const budget_for_new = async (byuser)=>{
